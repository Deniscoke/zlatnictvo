...
/* ===================== GLOBAL GOLD-RAYMARCHER BACKDROP ===================== */
// Same Hurrle box↔octahedron raymarcher as the intro, but skinned with a
// dark-onyx → deep-gold → champagne palette so the existing light text on the
// page stays readable. Pauses when the tab is hidden to preserve GPU/battery.
(function bootBgCanvas(){
  const canvas = document.getElementById('bg-canvas');
  if(!canvas) return;
  if(window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  const gl = canvas.getContext('webgl2');
  if(!gl) return; // browser without WebGL2 → falls back to body bg-onyx

  // Cap DPR at 1.5 — 4K monitors would otherwise render 4× the pixels for the
  // raymarcher loop (50 iterations per fragment). 1.5 looks crisp enough.
  ...
  // Same fragment shader as intro, dark-gold palette at the end.
  const fs = `#version 300 es
    precision highp float;
    out vec4 fragColor;
    uniform vec2 resolution;
    ...
    void main(){
      ...
      // ---- Dark-gold palette: onyx → deep gold → champagne ----
      float lum = clamp(dot(col, vec3(.299,.587,.114)), 0., 1.);
      vec3 onyx      = vec3(0.045, 0.040, 0.035);   // near-black background
      vec3 deepGold  = vec3(0.260, 0.180, 0.060);   // burnt copper-gold
      vec3 midGold   = vec3(0.620, 0.450, 0.140);   // rich gold
      vec3 champagne = vec3(0.950, 0.800, 0.420);   // bright highlight
  ...
  const compile = (type, src)=>{
    ...
    if(!gl.getShaderParameter(sh, gl.COMPILE_STATUS)){
      console.error('[bg-canvas]', gl.getShaderInfoLog(sh));
      return null;
    }
    return sh;
  };
  ...
  if(!gl.getProgramParameter(prog, gl.LINK_STATUS)){
    console.error('[bg-canvas]', gl.getProgramInfoLog(prog));
    return;
  }
  ...
  const draw = (now)=>{
    if(!running) return;
    gl.useProgram(prog);
    // Slow time multiplier (×0.6) — full-page motion is more soothing if subtle
    gl.uniform1f(uTime, (now - start) * 0.0006);
    gl.uniform2f(uRes, canvas.width, canvas.height);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
  ...
  rafId = requestAnimationFrame(draw);
  // Reveal once the first frame has drawn (avoids flash of black)
